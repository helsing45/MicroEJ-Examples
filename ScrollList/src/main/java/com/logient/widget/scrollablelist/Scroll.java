 ...
 */
package com.logient.widget.scrollablelist;
...
public class Scroll extends Container {
		...
		if (this.child instanceof Container) {
			...
			if (type == Pointer.EVENT_TYPE) {
				...
				if (action == Buttons.RELEASED) {
					...
					Container childContainer = (Container) this.child;
					int childCount = childContainer.getChildrenCount();
					for (int i = 0; i < childCount; i++) {
						Widget child = childContainer.getChild(i);
						if (pointerIsOnChild(child, pointer.getX(), pointer.getY())
								&& childContainer.getChild(i).handleEvent(event)) {
							return true;
						}
					}
				}
	...
	private boolean pointerIsOnChild(Widget child, int x, int y) {
		int startYBound = child.getAbsoluteY();
		int endYBound = startYBound + child.getHeight();
		int startXBound = child.getAbsoluteX();
		int endXBound = startXBound + child.getWidth();

		return startYBound <= y && y <= endYBound && startXBound <= x && x <= endXBound;
